...
describe('Stage 5 Workflow: Package → Environment Drop', () => {
  ...
  const initialData: UnmergedAsset[] = [
    ...
    { id: 'webserver-pkg-a', fqn: 'EnvA::WebServer', assetType: ASSET_TYPES.PACKAGE, assetKey: 'WebServer', templateFqn: 'SharedBase', overrides: { port: 8080 } },
  ...
  it('should prompt for confirmation with the correct inheritance comparison', () => {
    ...
    // ASSERT: The generic core uiStore should be in a state to show a dialog
    expect(uiStore.dragDropConfirmationDialog.show).toBe(true);

    // Assert that the content layer provided the correct specific data
  ...
  it('should call the correct execution action upon dialog confirmation', () => {
    ...
    uiStore.promptForDragDropConfirmation({
        dragPayload,
        dropTarget,
        displayPayload: { type: 'CrossEnvironmentCopy', inheritanceComparison: { before: [], after: [] } }
    });
    ...
    executePoolCopy(uiStore.dragDropConfirmationDialog.dragPayload!, uiStore.dragDropConfirmationDialog.dropTarget!);
